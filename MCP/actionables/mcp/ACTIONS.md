...
- `cpu_percent(interval=1)` blocks for 1 second — this is intentional. Without an interval, psutil returns 0.0 on the first call since it has no previous sample to compare against.
- Always use an **absolute path** in the `claude mcp add` command.
- If you update `server.py` (add/rename tools), run `claude mcp list` and then start a fresh Claude Code session — it won't pick up changes in an already-running session.
- Do **not** print anything to stdout in `server.py`. The MCP protocol uses stdout for communication — any stray print will corrupt the message and break the server. Use `sys.stderr` if you need to debug.
